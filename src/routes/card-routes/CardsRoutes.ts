...
const CardsRouter: FastifyPluginAsync = async (
  ...
  fastify.put(
    "/cards/:cardID",
    { schema: updateCardSchema, preHandler: [AuthMiddleware, RoleMiddleware(['superadmin', 'admin', 'owner', 'director', 'head_marketing', 'head_accountant', 'head_cashier'])] },
    UpdateCardsController,
  );

  fastify.delete(
    "/cards",
    { schema: deleteCardsSchema, preHandler: [AuthMiddleware, RoleMiddleware(['superadmin', 'admin', 'owner', 'director', 'head_marketing', 'head_accountant', 'head_cashier'])] },
